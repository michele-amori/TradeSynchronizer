...
# New-order placement uses the plural "/orders" — empirically
# confirmed against real TradingView ↔ IBKR traffic captured by the
# proxy during calibration. The trailing `(?:\?.*)?$` makes the
# regex tolerant of query strings — TV may or may not append them
# (it sticks `?manualIndicator=true` on DELETE, see below); having
# the same tolerance everywhere keeps the pattern future-proof.
_NEW_ORDER_PATH_RE = re.compile(
    r"^/v1/tv/iserver/account/(?P<acct>[\w]+)/orders(?:\?.*)?$"
)

# Per-order management — cancel (DELETE) and modify (POST / PUT) —
# uses the SINGULAR "/order/{order_id}". The asymmetry is a real
# IBKR API quirk: place is `/orders` (plural), then every operation
# on a specific existing order is `/order/{id}` (singular).
# `order_id` is constrained to digits to avoid matching the
# sibling endpoint /order/whatif (preview request).
# `(?:\?.*)?$` accepts an optional query string — TradingView
# appends `?manualIndicator=true` to DELETE requests (observed
# live), and possibly other params we haven't seen yet.
_SINGLE_ORDER_PATH_RE = re.compile(
    r"^/v1/tv/iserver/account/(?P<acct>[\w]+)/order/(?P<order_id>\d+)(?:\?.*)?$"
)

# `/v1/tv/iserver/account/orders` (note: no `account/<id>/orders` —
# this is the account-agnostic top-level list endpoint with an
# `accountId` query param). TradingView polls this periodically (and
# also right after placing a bracket) to learn the IBKR order ids of
# every leg, including the bracket children that the original POST
# /orders response only acknowledged via the entry. The addon uses
# this response to bind those child IBKR ids back to the synthetic
# cOIDs registered by the replicator at placeoso time.
_ORDERS_LIST_PATH_RE = re.compile(
    r"^/v1/tv/iserver/account/orders(?:\?.*)?$"
)
...
class IbkrOrderModify:
    ...
    `order_type` is required by Tradovate's modifyorder endpoint
    (it'll respond HTTP 400 "missing required field orderType" if
    we forget). TradingView always seems to include it in the
    modify body, so we surface it explicitly here instead of
    forcing the replicator to dig through `raw`.
    """
    account_id:    str
    ibkr_order_id: str
    ...
    tif:           Optional[str]
    order_type:    Optional[str]    # "MKT" | "LMT" | "STP" | "STP LMT"
    raw:           dict
...
def is_orders_list_response(flow: http.HTTPFlow) -> bool:
    """True when this is a successful GET to /v1/tv/iserver/account/orders.

    The addon listens to this so it can pick up the IBKR order ids of
    bracket children — which the original POST /orders response only
    acknowledges via the entry, leaving subsequent modify/cancel
    requests on the children unbindable to anything in our order map
    until this poll fires.
    """
    ...
    if flow.response is None or flow.response.status_code != 200:
        return False
    return bool(_ORDERS_LIST_PATH_RE.match(flow.request.path))


def parse_orders_list_bracket_children(
    response_body: bytes,
) -> list:
    """Extract (parent_ibkr_id, child_ibkr_order_id, child_role) tuples.

    IBKR's GET /v1/tv/iserver/account/orders returns a JSON object
    with an `orders` array. Each element looks like:

        {
          "orderId":   77461943,        # IBKR-side numeric id
          "cOID":      "<entry's cOID>"  # for the parent (entry)
          "parentId":  77461943          # for a child — the entry's
                                         # IBKR ORDER ID, not its cOID
          "orderType": "LMT"|"STP"|...,
          ... other fields not relevant here
        }

    NOTE: an earlier revision of this docstring claimed `parentId`
    was the parent's cOID. That was wrong — verified empirically on
    2026-06-07 with a real bracket-MKT placement: a child with
    parentId=319073567 referenced an entry whose cOID was
    "xsPj1DgsevTF" and whose IBKR orderId was 319073567. So the
    `parentId` field carries the parent's IBKR order id. Callers
    that need the entry's cOID (e.g. to form the synthetic key the
    replication engine uses) must resolve parent IBKR id → cOID via the
    order map (see OrderMap.coid_for_ibkr_id).

    Field names vary across IBKR versions (some payloads use
    snake_case `order_id` / `parent_id` / `order_type`), so we
    tolerate both.

    Returns a list of (parent_ibkr_id, ibkr_order_id, role)
    tuples, where role is the normalized bracket-child role token
    ('LMT' | 'STP' | 'STPLMT').

    Silent on parse failure: returns []. The caller treats this as
    "no children to bind right now" which is correct — the next
    /orders poll will retry.
    """
    ...
    for o in orders:
        if not isinstance(o, dict):
            continue
        parent_ref = o.get("parentId") or o.get("parent_id")
        if not parent_ref:
            continue
        ibkr_order_id = o.get("orderId") or o.get("order_id")
        if ibkr_order_id is None:
            continue
        order_type = o.get("orderType") or o.get("order_type")
        if not order_type:
            continue
        # Normalize to the same role tokens the replicator's
        # _bracket_child_role() emits.
        t = str(order_type).upper().strip()
        if t in ("LMT", "LIMIT"):
            role = "LMT"
        elif t in ("STP", "STOP"):
            role = "STP"
        elif t in ("STP LMT", "STOPLIMIT", "STP_LMT"):
            role = "STPLMT"
        else:
            continue
        out.append((str(parent_ref), str(ibkr_order_id), role))
    return out
...
def _parse_bracket(orders: list) -> IbkrBracket:
    ...
    for parent_id, child in child_legs:
        ...
        # TV's bracket-child convention is asymmetric: for STP/STP LMT
        # legs the stop TRIGGER price arrives in `price`, NOT in
        # `auxPrice` (which TV omits entirely). Compare this with a
        # standalone STP modify body, where TV sends `price` and
        # `auxPrice` both set to the trigger. We normalise to the
        # IBKR-canonical shape so downstream code (and the existing
        # placement helpers in tradesync.brokers.tradovate) can treat
        # every STP the same way: aux_price = trigger, price = limit
        # (limit being None for pure STP, populated for STP LMT).
        # First captured live during calibration: bracket SL child
        # `{orderType: STP, price: 30126.5}` (no auxPrice) was
        # forwarded to Tradovate with stop_price=None and rejected
        # HTTP 400 "Stop Price should be specified".
        child_price = _to_float(child.get("price"))
        child_aux   = _to_float(child.get("auxPrice"))
        if order_type == "STP" and child_aux is None and child_price is not None:
            child_aux, child_price = child_price, None
        elif order_type == "STP LMT" and child_aux is None and child_price is not None:
            # Degenerate STP LMT — TV didn't send a separate limit.
            # Use the same value for both; if that's truly wrong the
            # user will see it in the resulting Tradovate order and
            # we'll have logs to refine the heuristic.
            child_aux = child_price
...
def parse_ibkr_modify(flow: http.HTTPFlow) -> IbkrOrderModify:
    ...
    tif = o.get("tif")
    ot_raw = o.get("orderType")
    if isinstance(ot_raw, str):
        ot = ot_raw.strip().upper()
        # Canonicalise STPLMT → STP LMT to match the new-order parser.
        if ot == "STPLMT":
            ot = "STP LMT"
    else:
        ot = None
