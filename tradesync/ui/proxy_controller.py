...
def _is_apple_silicon_hardware() -> bool:
    ...
    Unlike platform.machine() / os.uname().machine, this detection
    is robust against the caller running under Rosetta translation
    (which makes both of those return "x86_64" even on M1/M2/M3
    hardware). The kernel sysctl hw.optional.arm64 always returns
    "1" on Apple Silicon regardless of caller arch.
    """
