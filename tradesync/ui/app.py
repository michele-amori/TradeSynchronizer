...
# Persistence and subprocess-control logic now live in their own
# modules; re-exported here so existing imports
# (`from tradesync.ui.app import EnvStore` / `ProxyController`) keep
# working unchanged.
from .env_constants import ENVIRONMENTS, PER_ENV_DEFAULTS, PER_ENV_KEYS
from .env_store import (  # noqa: F401 - re-exported for back-compat
    EnvStore,
    SHARED,
    _CANONICAL_SHARED_KEYS,
    _FILE_BUCKETS,
)
from .proxy_controller import (  # noqa: F401 - re-exported for back-compat
    ProxyController,
    _is_apple_silicon_hardware,
)


def _has_enabled_tradovate_source_pair(project_root, env: str) -> bool:
    """True iff replication.json has an ENABLED pair whose SOURCE is
    Tradovate in the given environment ('live' / 'demo').

    Used to decide whether the engine subprocess for this env should run
    the Tradovate→IBKR (WS-pipeline) direction. Returns False on any
    error (no config, parse failure) so a missing/broken config simply
    leaves that direction off rather than crashing the launch.
    """
    try:
        ...
        return any(
            p.source.broker == "tradovate" and p.source.env == env
            for p in cfg.enabled_pairs
        )
    except Exception:  # noqa: BLE001 - never block engine launch
        return False
...
# ─────────────────────────────────────────────────────────────────────── #
#
# ENVIRONMENTS / PER_ENV_KEYS / PER_ENV_DEFAULTS are imported from
# env_constants (above); _CANONICAL_SHARED_KEYS lives in env_store.
# They're used both by the form declarations below and by EnvStore.
...
GENERAL_FIELDS: list[tuple] = [
    ...
    ("TRADOVATE_IS_AUTOMATED", "Mark orders as automated", "bool", "false",
        "Sent as the `isAutomated` field on every order payload to "
        "Tradovate. Default OFF — most trade-copier services (e.g. "
        "TradeSyncer.com) skip orders with isAutomated=true by "
        "default, so leaving this ON breaks the leader→follower "
        "fanout silently (the leader accepts the order, the copier "
        "ignores it). Turn ON only if your upstream is genuinely "
        "an autonomous algo and you want it labelled as such for "
        "regulatory or reporting purposes; the leader account "
        "accepts both values equivalently."),
...
PER_ENV_FIELDS: list[tuple] = [
    # NOTE: the "Tradovate account" section (TRADOVATE_USERNAME /
    # PASSWORD / ACCOUNT_ID + the "Sign in & pick account" picker) used
    # to live here. It was removed: the user writes those three values by
    # hand into .env.live / .env.demo, EnvStore preserves them verbatim
    # (they're no longer in PER_ENV_KEYS), and config.py reads them from
    # the environment. ACCOUNT_ID accepts the numeric id OR the account
    # name (e.g. DEMO3701228); the engine resolves a name to the internal
    # id via Tradovate's /account/list at startup, so the old GUI picker
    # is no longer needed.
    ...
    # NOTE: the "IBKR account to mirror" field (IBKR_WATCHED_ACCOUNTS)
    # used to live here. It moved to the Replication tab: an IBKR-source
    # pair's source account now drives the watch list. config.py still
    # reads IBKR_WATCHED_ACCOUNTS as a backward-compat fallback for env
    # files that predate the pair model.
]
...
class TradeSyncApp:
    ...
    def _build_ui(self):
        ...
        # Replication direction picker (bidirectional source→follower
        # pairs). Built in its own module; failure to build it must not
        # take down the rest of the GUI, so guard the import/build.
        try:
            from .replication_settings import (
                ReplicationSettingsController, build_panel,
            )
    ...
    def _build_per_env_tab(self, parent, env: str):
        ...
        # Orientation header: make the split explicit — this tab holds
        # this environment's LOGIN + proxy settings, while the routing
        # ("what mirrors where") lives on the Replication tab.
        intro = ttk.Frame(parent)
        intro.pack(fill="x", padx=12, pady=(10, 0))
        ttk.Label(intro, text=f"{env.capitalize()} logins & proxy",
                  font=("", 13, "bold")).pack(anchor="w")
        ttk.Label(
            intro,
            text=("This environment's Tradovate login and proxy port. "
                  "Which account mirrors onto which is set on the "
                  "Replication tab, not here."),
    ...
    def _autofix_port_collision(self) -> Optional[str]:
        """Reset PROXY_LISTEN_PORT to the canonical defaults when both
        .env.live and .env.demo are configured for the same port.

        Two engine subprocesses can't bind the same TCP port, so the
        validator rejects the collision at Start time with a confusing
        "Port X is configured for both LIVE and DEMO" error. We can do
        better: silently force LIVE→8080 and DEMO→8081 (the values in
        PER_ENV_DEFAULTS / Config.load's fallback) and persist them so
        the user never sees the warning again across restarts.

        Returns a short human-readable notice if anything was changed,
        None when there was no collision to fix. Called once at GUI
        boot, right after EnvStore.load().
        """
        live_port = (self.store.per_env["live"].get("PROXY_LISTEN_PORT") or "").strip()
        demo_port = (self.store.per_env["demo"].get("PROXY_LISTEN_PORT") or "").strip()
        # Only auto-fix when BOTH are explicitly set AND identical. If
        # either is blank, Config.load picks the default later — no
        # collision in practice.
        if not live_port or not demo_port or live_port != demo_port:
            return None
        canonical_live = PER_ENV_DEFAULTS["PROXY_LISTEN_PORT"]["live"]
        canonical_demo = PER_ENV_DEFAULTS["PROXY_LISTEN_PORT"]["demo"]
        self.store.per_env["live"]["PROXY_LISTEN_PORT"] = canonical_live
        self.store.per_env["demo"]["PROXY_LISTEN_PORT"] = canonical_demo
        # Persist now so the next GUI start sees the corrected files
        # and doesn't re-trigger this branch on a stale collision.
        self.store.write(only={"live", "demo"})
        return (f"Both engines were configured for port {live_port}. "
                f"Reset to canonical defaults: LIVE={canonical_live}, "
                f"DEMO={canonical_demo}. Saved to .env.live / .env.demo.")

    def _load_settings(self):
        self.store.load()
        port_notice = self._autofix_port_collision()
        if port_notice:
            # Surface it in the Log tab — the same channel the engines
            # use, so it can't be missed. No modal popup: this is a
            # benign fix-up, not a user-facing error.
            self.log_q.put(f"[GUI] ⚙ Port collision auto-fixed — "
                           f"{port_notice}\n")
    ...
    def _on_start(self, env: str):
        ...
        # Enable the Tradovate→IBKR (WS-pipeline) direction for this
        # engine ONLY when there's an enabled Tradovate-source pair for
        # this environment. Without this the subprocess never receives
        # TRADESYNC_ENABLE_WS_PIPELINES and that direction silently never
        # starts — only the IBKR→Tradovate proxy hot path runs. The flag
        # is scoped per-env so a DEMO pair can't switch on the LIVE
        # engine's replication, or vice versa.
        if _has_enabled_tradovate_source_pair(self.project_root, env):
            env_overrides["TRADESYNC_ENABLE_WS_PIPELINES"] = "1"
