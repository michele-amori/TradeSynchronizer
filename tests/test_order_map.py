...
class TestNeutralApi(unittest.TestCase):
    """The broker-neutral aliases must delegate to the historical
    methods so there's exactly one implementation, and the neutral
    record views must reflect the stored fields."""

    def test_record_neutral_views(self):
        from tradesync.order_map import OrderRecord
        rec = OrderRecord(cOID="ENTRY1", ibkr_order_id="319073567",
                          tradovate_id=11978727757)
        self.assertEqual(rec.source_label, "ENTRY1")
        self.assertEqual(rec.source_order_id, "319073567")
        # follower id is surfaced as a string
        self.assertEqual(rec.follower_order_id, "11978727757")

    def test_follower_order_id_none_when_unplaced(self):
        from tradesync.order_map import OrderRecord
        rec = OrderRecord(cOID="ENTRY1")
        self.assertIsNone(rec.follower_order_id)

    def test_set_follower_id_then_resolve(self):
        with _TmpMap() as om:
            om.set_follower_id("ENTRY1", "11978727757")
            om.bind_source_id("ENTRY1", "319073567")
            # neutral resolution: source order id → follower order id (str)
            self.assertEqual(
                om.follower_id_for_source_id("319073567"), "11978727757")
            # neutral resolution: source order id → source label
            self.assertEqual(
                om.source_label_for_source_id("319073567"), "ENTRY1")

    def test_neutral_and_historical_are_same_store(self):
        # A write via the neutral API must be visible via the
        # historical API and vice-versa — they share one store.
        with _TmpMap() as om:
            om.set_follower_id("c1", "500")
            self.assertEqual(om.get_by_coid("c1").tradovate_id, 500)
            om.set_ibkr_id("c1", "ibkr-1")
            self.assertEqual(om.follower_id_for_source_id("ibkr-1"), "500")

    def test_record_for_source_label(self):
        with _TmpMap() as om:
            om.add_pending("c1")
            rec = om.record_for_source_label("c1")
            self.assertIsNotNone(rec)
            self.assertEqual(rec.source_label, "c1")

    def test_remove_by_source_id(self):
        with _TmpMap() as om:
            om.set_follower_id("c1", "500")
            om.bind_source_id("c1", "ibkr-1")
            om.remove_by_source_id("ibkr-1")
            self.assertIsNone(om.follower_id_for_source_id("ibkr-1"))
            self.assertIsNone(om.record_for_source_label("c1"))

    def test_remove_by_source_label(self):
        with _TmpMap() as om:
            om.set_follower_id("c1", "500")
            om.remove_by_source_label("c1")
            self.assertIsNone(om.record_for_source_label("c1"))
