...
class TestEnsureFlow(unittest.TestCase):
    ...
    def test_force_restart_default_relaunches_even_on_correct_port(self):
        """Default behaviour (force_restart=True): always quit and
        relaunch TV when an engine starts, even if TV is already
        running on the correct port. This clears any stale state
        from a previous engine session that got terminated mid-flow."""
        with patch.object(tvl, "is_installed", return_value=True), \
             patch.object(tvl, "_wait_for_port", return_value=True), \
    ...
    def test_force_restart_false_is_noop_when_already_proxied(self):
        """Legacy/opt-out behaviour: caller can pass
        force_restart=False to preserve the older "leave TV alone if
        it's already wired up" semantics (useful for tests and for
        any hot-reload path that doesn't involve a port change)."""
        with patch.object(tvl, "is_installed", return_value=True), \
             patch.object(tvl, "_wait_for_port", return_value=True), \
             patch.object(tvl, "is_running", return_value=True), \
             patch.object(tvl, "running_proxy_port", return_value=8081), \
             ...
             patch.object(tvl, "_quit_tradingview") as mock_quit:
            self.assertEqual(
                tvl.ensure_tradingview_via_proxy(8081, force_restart=False),
                "already_proxied",
            )
            mock_launch.assert_not_called()
            mock_quit.assert_not_called()
